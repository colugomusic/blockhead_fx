...
{
	return ((Saturator*)(handle))->get_name().c_str();
}

void rack_unit_process(void* handle, int num_frames)
{
	((Saturator*)(handle))->process(num_frames);
}

int rack_unit_get_num_params(void* handle)
{
	return ((const Saturator*)(handle))->get_num_params();
}

int rack_unit_get_num_input_channels(void* handle)
{
	return ((const Saturator*)(handle))->get_num_input_channels();
}

int rack_unit_get_num_output_channels(void* handle)
{
	return ((const Saturator*)(handle))->get_num_output_channels();
}

void* rack_unit_get_input_channel(void* handle, int id)
{
	return ((const Saturator*)(handle))->get_input_channel(id);
}

void* rack_unit_get_output_channel(void* handle, int id)
{
	return ((const Saturator*)(handle))->get_output_channel(id);
}

void* rack_unit_get_param(void* handle, int id)
{
	return ((const Saturator*)(handle))->get_param(id);
}

int rack_unit_get_sample_rate(void* handle)
{
	return ((const Saturator*)(handle))->get_sample_rate();
}

void rack_unit_set_sample_rate(void* handle, int sample_rate)
{
	((Saturator*)(handle))->set_sample_rate(sample_rate);
}

char rack_unit_copy(void* dest, void* source)
{
	((Saturator*)(dest))->copy(*((Saturator*)(source)));
